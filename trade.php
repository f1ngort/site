    ...
    if (!isset($_SESSION['user_id'])) {
        ...
        try {
            ...
            $data = [
                ...
                'fn' => $_SESSION['user_fn'] ?? '',
                'sn' => $_SESSION['user_sn'] ?? '',
                'zayavk' => 'Консультация по Trade-in',
                'status' => 'Новая',
            ...
            $_SESSION['success_message'] = "Заявка на консультацию по Trade-in успешно отправлена!";
...
<head>
    ...
    <title>Автоцетр Колесо - Trade-in</title>
...
<form id="consultationForm" class="hidden-form" method="POST">
    ...
    <main>
        <section class="Hero trade">
            <div class="lisingbox">
                <h3>Выгодные условия по TRADE-IN</h3>
                <button type="button" onclick="submitConsultation()">Получить консультацию</button>
            </div>
        </section>
        <section class="something">
            <div class="smtbox11"></div>
            <div class="smtbox2">
                <h2><span>Авто</span> по TRADE-IN</h2>
                <p>В авто центре «Колесо» Вы выбираете новую модель, после чего мы проводим оценку вашего текущего автомобиля, учитывая его техническое состояние, пробег и год выпуска, чтобы согласовать справедливую стоимость. Как только условия будут согласованы, вы снимаете старый автомобиль с учета, заключаете договор поручения, и после доплаты уезжаете на новом авто. На протяжении всего процесса — от оценки вашего старого автомобиля до получения нового — с вами работают продавец-консультант и специалист отдела Trade-In, которые берут на себя все вопросы, связанные с диагностикой, подбором новой модели и оформлением, учитывая ваши предпочтения. Ваш старый автомобиль может стать первоначальным взносом по кредиту, позволяя вам уехать на новом авто без дополнительных платежей. Мы полностью берем на себя все заботы по поиску покупателя, рекламе и оформлению документов, делая процесс продажи старого и покупки нового автомобиля максимально простым, безопасным и быстрым, экономя ваше время, силы и деньги.</p></div>
        </section>
